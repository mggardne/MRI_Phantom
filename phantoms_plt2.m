...
%
if ok<1
  warning(' *** WARNING in phantoms_plt2:  No series selected!');
  return
end
%
idv = find(idv);
...
%
k1 = find(etns>0.99&etns<1.01);        % Echo time = 1 ms (usually = 2)
...
%
   ...
   if rsize>6
     ...
     elseif rsize<6
       error([' *** ERROR in phantoms_plt:  Less than six regions', ...
              ' were identified!']);
     end
   elseif rsize<6
     error([' *** ERROR in phantoms_plt:  Less than six regions', ...
...
%
mf = ['phantoms' snt '_plt2.mat'];     % MAT file name
...
%
print('-dpdf','-r600','-fillpage',['phantoms' snt '_plt2.pdf']);
%
return